...
describe('ProgressIndicator', () => {
  it('renders semantic steps with current state', () => {
    render(<ProgressIndicatorRoot steps={steps} defaultStep={1} label="Checkout steps" />)
  ...
  it('blocks future click targets in linear mode', async () => {
    ...
    render(<ProgressIndicatorRoot steps={steps} defaultStep={1} onStepChange={onStepChange} />)
  ...
  it('navigates to any step in non-linear mode', async () => {
    const user = userEvent.setup()
    const onStepChange = vi.fn()
    render(<ProgressIndicatorRoot steps={steps} linear={false} onStepChange={onStepChange} />)
  ...
  it('supports keyboard activation', async () => {
    const user = userEvent.setup()
    render(<ProgressIndicatorRoot steps={steps} defaultStep={1} />)
