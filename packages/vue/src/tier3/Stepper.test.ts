...
describe('Stepper', () => {
  it('renders a labelled spinbutton and boundary controls', () => {
    render(h(StepperRoot, { label: 'Quantity', min: 1, max: 5, defaultValue: 1 }))
  ...
  it('increments and decrements value', async () => {
    ...
    render(h(StepperRoot, { label: 'Quantity', min: 1, max: 5, defaultValue: 1, onValueChange }))
  ...
  it('supports keyboard spinbutton shortcuts', async () => {
    const user = userEvent.setup()
    render(h(StepperRoot, { label: 'Quantity', min: 1, max: 10, defaultValue: 2, largeStep: 5 }))
  ...
  it('renders formatted value text and hidden form input', () => {
    render(
      h(StepperRoot, {
        ...
        max: 5,
        defaultValue: 3,
        formatValue: (value: number) => `${value} tickets`,
      }),
    )
  ...
  it('reflects controlled value', async () => {
    const user = userEvent.setup()
    const onValueChange = vi.fn()
    render(h(StepperRoot, { label: 'Quantity', min: 1, max: 5, value: 3, onValueChange }))
    await user.click(screen.getByRole('button', { name: 'Increase' }))
