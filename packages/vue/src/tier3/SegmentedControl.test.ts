...
import { SegmentedControlRoot, type SegmentedControlItem } from './SegmentedControl'
...
describe('SegmentedControl', () => {
  it('renders radio mode by default', () => {
    render(h(SegmentedControlRoot, { items: ITEMS, defaultValue: 'grid', name: 'view' }))
  ...
  it('changes radio selection', async () => {
    ...
    render(h(SegmentedControlRoot, { items: ITEMS, defaultValue: 'grid', onValueChange }))
  ...
  it('renders tab mode', () => {
    render(h(SegmentedControlRoot, { items: ITEMS, defaultValue: 'grid', mode: 'tabs' }))
  ...
  it('reflects controlled value', async () => {
    const user = userEvent.setup()
    const onValueChange = vi.fn()
    render(h(SegmentedControlRoot, { items: ITEMS, value: 'grid', onValueChange }))
    await user.click(screen.getByRole('radio', { name: 'List' }))
    expect(screen.getByRole('radio', { name: 'Grid' })).toBeChecked()
    expect(onValueChange).toHaveBeenCalledWith('list', 'grid')
  })
